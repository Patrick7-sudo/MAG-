...
const airportSearch = express.Router();

airportSearch.get("/", async(req,res)=>{
    ...
    async function getIndividualRoute(input) {
      const data = await query(
        `SELECT CONCAT(airlines.iata_code, ' ',flight_num) AS "Flight Number",
          ...
          JOIN airport AS arrivalport ON arrival = arrivalport.id WHERE departport.location_code_iata = $1 OR departport.name =$1`,
        [input]
      );
